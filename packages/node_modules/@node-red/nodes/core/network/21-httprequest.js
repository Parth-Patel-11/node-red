    ...
    function HTTPRequest(n) {
        ...
        */
        ...
        this.on("input",function(msg,nodeSend,nodeDone) {
            ...
            opts.method = method;
            opts.retry = {
                limit: n.retry || 0, // number of retries
                methods: ['GET','POST','PUT','DELETE','PATCH'],
                statusCodes: [408,429,500,502,503,504],
                errorCodes: ['ETIMEDOUT','ECONNRESET','EAI_AGAIN','ECONNREFUSED','ENOTFOUND','ESOCKETTIMEDOUT'],
                calculateDelay: ({ attemptCount, error, computedValue }) => {
                    node.status({ fill:"yellow", shape:"ring", text:`Retry ${attemptCount}/${n.retry} (${error.code})` });
                    node.warn(`[HTTP Retry] Attempt ${attemptCount} - ${error.code || error.message}`);
                    return computedValue; // exponential backoff
                }
            };
            ...
            got(url,opts).then(res => {
                ...
                // If we have no more requests in progress, clear the status
                if (node.count === 0) {
                    node.status({});
                }
                nodeSend(msg);
